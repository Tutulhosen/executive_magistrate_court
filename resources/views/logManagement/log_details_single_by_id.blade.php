...
@php
    ...
    if ($log_details_single_by_id->case_basic_info) {
        ...
        $division = DB::table('division')
            ->where('id', '=', $case_basic_info->division)
            ->first();
        $district = DB::table('district')
            ->where('id', '=', $case_basic_info->district)
            ->first();
        $upazila = DB::table('upazila')
            ->where('id', '=', $case_basic_info->upazila)
            ->first();
    } else {
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>বিগত ইং {{ en2bn($case_basic_info->caseDate) }} তারিখ মোতাবেক বাংলা
                                        {{ BnSal($data->log_details_single_by_id->created_at, 'Asia/Dhaka', 'j F Y') }}
                                        সময়:
