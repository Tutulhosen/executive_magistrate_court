...
class LoginController extends Controller
{
    ...
    public function csLogin(Request $request)
    {
        ...
        if (Auth::attempt(['email' => $request->email, 'password' => $request->password])) {
            ...
            if (mobile_first_registration()) {
                $user = User::where('mobile_no', $request->email)->where('otp', $request->password)->first();
                if(!empty($user))
                {
                    if (Auth::attempt(['mobile_no' => $request->email, 'password' => 'google_sso_login_password_14789_gcc_ourt_otp_based'])) {
                        return response()->json(['success' => 'Successfully logged in!']);
                    }
                }
    ...
    public function ndoptor_sso_nothi_issus(Request $request)
    {
        ...
        $data['message2'] = 'আপনার নথির তথ্য সহ আপনার লগইন সফল হয়েছে। কিন্তু আপনাকে এখনো এক্সিকিউটিভ ম্যাজিস্ট্রেট কোর্ট  আদালতে নিযুক্ত করা হয়নি। নিযুক্তি নিশ্চিত করতে  01XXXXXXXXX নাম্বারে যোগাযোগ করুন।';
        //Cookie::queue(Cookie::forget('_ndortor'));
        $callbackurl = url('/');
        $zoom_join_url = DOPTOR_ENDPOINT() . '/logout?' . 'referer=' . base64_encode($callbackurl);

        $data['callbackurl'] =getCommonBaseUrl().'/doptor/court';// $zoom_join_url;
