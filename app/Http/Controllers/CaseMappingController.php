...
class CaseMappingController extends Controller
{
    ...
    public function index()
    {
        ...
        $available_court = DB::table('court')->where('division_id', $officeInfo->division_id)->where('district_id', $officeInfo->district_id)->whereIn('level', [0, 1])->get();
    ...
    {
        $court_type = DB::table('court')->where('id', '=', $request->court_id)->first();
        $data = $request->all();
        $officeInfo = user_office_info();
        ...
        if ($token) {
            ...
            if ($res->status == 'success') {
                ...
                if ($court_type->level == 0) {
                    $officeInfo = user_office_info();
    
                    $assignedupzilla = DB::table('case_mapping')->where('court_id', $request->court_id)->where('district_id', $officeInfo->district_id)->where('status', 1)->where('lavel', 0)->select('upazilla_id')->get();
                    ...
                    if (!empty($request->upzilla_case_mapping)) {
                        ...
                        foreach ($request->upzilla_case_mapping as $up_id) {
                            ...
                            if (!in_array($up_id, $assignedipzilla)) {
                                ...
                                $case_id = DB::table('em_appeals')->where('upazila_id', $up_id)->where('district_id', $officeInfo->district_id)
                                    ->select('id')->get();
                                // dd($case_id);
                                if (!empty($case_id)) {
                    ...
                    foreach ($assignedipzilla as $up_id) {
                        ...
                        if (!in_array($up_id, $requestupzill)) {
                            ...
                            //echo $up_id;
                            $previous_court_id = DB::table('em_appeals')->where('upazila_id', $up_id)->where('district_id', $officeInfo->district_id)
                                ->select('court_id')->get();
                    ...
                    $assignedupzilla = DB::table('case_mapping')->where('court_id', $request->court_id)->where('district_id', $officeInfo->district_id)->where('status', 1)->where('lavel', 1)->select('upazilla_id')->get();
                    $assignedipzilla = array();
                    ...
                    foreach ($assignedipzilla as $up_id) {
                        ...
                        if (!in_array($up_id, $requestupzill)) {
                            ...
                            //echo $up_id;
                            $previous_court_id = DB::table('em_appeals')->where('upazila_id', $up_id)->where('district_id', $officeInfo->district_id)->whereIn('law_section', [6, 7])->select('court_id')->get();
